...
class VehicleTracker:
    """Advanced vehicle tracker using Kalman filtering and optical flow."""
    ...
    def _match_detections(
        ...
        """Match detections to existing tracks using greedy algorithm."""
        ...
        # Build cost matrix
        track_centers = [t.center for _, t in active_tracks]
        track_ids = [tid for tid, _ in active_tracks]
        ...
        for i, (tx, ty) in enumerate(track_centers):
            for j, (dx, dy) in enumerate(current_centers):
                cost_matrix[i, j] = np.sqrt((tx - dx) ** 2 + (ty - dy) ** 2)
    ...
    def update(self, frame: np.ndarray, detections: List[Dict]) -> List[Dict]:
        ...
        for track_id, det_idx in matches.items():
            ...
            if self.kalman_enabled and track_id in self.kalman_filters:
                ...
                pred_x = float(prediction[0]) if prediction.ndim == 1 else float(prediction[0, 0])
                pred_y = float(prediction[1]) if prediction.ndim == 1 else float(prediction[1, 0])
        ...
        for track_id, track in self.tracks.items():
            if current_time - track.last_seen > self.max_lost_frames * 0.1:
                track.active = False
                inactive_tracks.append(track_id)

        # Remove oldest inactive tracks
    ...
    def _calculate_speed(self, track: TrackedVehicle) -> float:
        ...
        # Convert to km/h (calibration: 1 pixel ≈ 0.05 meters)
        ...
        alpha = 0.7
        if hasattr(track, 'smoothed_speed') and track.smoothed_speed > 0:
